  ...
  return (
    <motion.nav className="fixed top-0 right-0 p-4 z-50">
      ...
      >
        <AnimatePresence>
          <motion.div
            className={`rounded-lg shadow-lg flex ${isMobile ? 'flex-row justify-around' : 'flex-col items-center space-y-4'} overflow-hidden ${
          ...
          >
            ...
            {/* Links */}
            <motion.ul
              className={`flex ${isMobile ? 'flex-row justify-around w-full' : 'flex-col items-center space-y-2'} text-white ${isMobile ? 'p-2' : 'mt-4'}`}
              initial={{ opacity: 0 }}
              animate={{ opacity: isMobile ? 1 : (isHovered ? 1 : 0) }}
              transition={{ duration: 0.3, delay: isHovered ? 0.3 : 0 }}
            >
              {["Home", "About", "Info", "Contact"].map((item) => (
                <li key={item} className={isMobile ? 'mx-2' : ''}>
